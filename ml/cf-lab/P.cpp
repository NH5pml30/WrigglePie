...
int main()
{
  ...
  int k1, k2;
  std::cin >> k1 >> k2;
  std::vector<std::map<int, int>> x2ys(k1);
  ...
  auto entropy = [](std::ranges::range auto &&r) {
    int sum = fold(r);
    double res = 0;
    for (int cnt : r)
    {
  ...
  std::cout << std::setprecision(10)
            << fold(x2ys | std::views::transform([noof_objs, &entropy](auto &ys) {
                      return entropy(ys | std::views::elements<1>) / noof_objs;
                    }))
            << std::endl;
  return 0;
}
