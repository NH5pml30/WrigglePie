...
private:
  ...
  int createNode(const std::vector<labeled_obj_t> &dataset, int noof_labels,
                 std::vector<std::vector<int>> &fid2inds, int ind_begin, int ind_end, int max_depth,
                 bool optimize, int depth = 0)
  {
    ...
    int min_i_end = -1;
    {
      ...
      for (int fid = 0; fid < noof_features; fid++)
      {
        ...
        for (int i_begin = 0; i_begin < noof_objs; i_begin = i_end, i++)
        {
          ...
          {
            int label = ys[i_end];
            phi[0].change(bycounts[0][label]++, +1);
            phi[1].change(bycounts[1][label]--, -1);
          }
          ...
          auto val = i_end * 1.0 / noof_objs * phi[0]() + (1 - i_end * 1.0 / noof_objs) * phi[1]();
          if (val < min_val)
          {
            min_val = val;
    ...
    int to_right = createNode<Estor>(dataset, noof_labels, fid2inds, ind_begin + min_i_end, ind_end,
...
public:
  template<typename Estor>
  DecisionTree(const std::vector<labeled_obj_t> &dataset, int noof_labels, std::type_identity<Estor>,
...
public:
  ...
  void change(int old_cnt, int delta)
  {
    sum += delta;
...
int main()
{
  ...
  auto tree = DecisionTree(train, noof_labels, std::type_identity<GainEstor>{}, max_depth,
                           train.size() > 3000);
