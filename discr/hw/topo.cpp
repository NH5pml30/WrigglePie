...
polynom tree_chroma(size_t n)
{
  polynom t = { 0, 1 }, t_1 = { -1, 1 }, res = t;
  for (size_t i = 0; i < n - 1; i++)
    res = multiply(res, t_1);
  return res;
}

std::string to_string(graph_t &g)
{
  std::stringstream ss;
  g.print_undirected(ss);
  return ss.str();
}

int main()
{
  std::ifstream file("graph5c.g6");
  g6::storage stg(file);

  std::map<polynom, std::vector<graph_t>> poly2g;
  for (graph_t g = stg.next_graph(); g.size() != 0; g = stg.next_graph())
  {
    polynom p = chroma_poly(g);
    if (p != tree_chroma(g.size()))
      poly2g[p].push_back(g);
  }

  for (auto &data : poly2g)
    if (data.second.size() > 1)
    {
      std::cout << "Check for isomorphism!" << std::endl;

      for (auto &g : data.second)
        std::cout << to_string(g) << std::endl << std::endl;
    }
