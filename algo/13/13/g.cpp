...
int main() {
  ...
  auto arr = find_suffix_array<int>(concat);
  auto lcp = buildLCP(concat, arr);

  int ans = 0, ans_i = -1;
  for (int i = 0; i < (int)lcp.size(); i++)
    if ((arr[i] < (int)s.size() && arr[i + 1] > (int)s.size()) ||
        (arr[i + 1] < (int)s.size() && arr[i] > (int)s.size()))
      if (lcp[i] > ans)
        ans = lcp[i], ans_i = i;

  if (ans_i >= 0)
    std::cout << std::string_view(concat.data() + arr[ans_i], ans);
  std::cout << std::endl;
  return 0;
}
