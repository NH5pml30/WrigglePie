...
graph_t read_graph(std::istream &i) {
  ...
  for (int j = 0; j < M; j++) {
    int adj1, adj2;
    i >> adj1 >> adj2;
    g.add_edge(adj1 - 1, adj2 - 1, {false});
    g.add_edge(adj2 - 1, adj1 - 1, {false});
  }
  return g;
}
    ...
    [&](graph_t::EDGE_PROXY from_parent, const std::vector<graph_t::EDGE_PROXY> &to_children) {
      ...
      for (auto child_edge : to_children)
        if (child_edge.edge_data.is_in_walk) {
          from_parent.to.data.up =
              std::min(from_parent.to.data.up, child_edge.to.data.up);
          normal_kids += child_edge.to.data.up >= from_parent.to.data.depth;
        }
      if (normal_kids > 0 && !(!from_parent.from.is_valid() && normal_kids == 1))
        artic_points.insert(from_parent.to.id);
    },
    [&]() {});

  std::cout << artic_points.size() << std::endl;
  for (auto pt : artic_points)
    std::cout << pt + 1 << ' ';
  std::cout << std::endl;
  return 0;
}
