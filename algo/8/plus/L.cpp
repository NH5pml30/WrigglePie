...
struct NODE_DATA_T {
  int came_from_id, depth, up, stack_size, popped_off;
};
struct EDGE_DATA_T {
  int id;
  bool is_in_walk;
...
int main() {
  ...
  std::vector<int> id2comp(g.n_edges() / 2);
  ...
  g.dfs(
    [&](graph_t::EDGE_PROXY from_parent) {
      ...
      case graph_t::color::white:
        ...
        from_parent.to.data.popped_off = 0;
        break;
      case graph_t::color::gray:
        if (from_parent.from.data.came_from_id != from_parent.to.id)
          from_parent.from.data.up = std::min(from_parent.from.data.up, from_parent.to.data.depth);
        break;
      ...
      if (from_parent.from.is_valid() && from_parent.from.data.came_from_id != from_parent.to.id) {
        ...
        current_comp.push(from_parent.edge_data.id);
      }
      return false;
    },
    [&](graph_t::EDGE_PROXY from_parent, const std::vector<graph_t::EDGE_PROXY> &) {
      if (from_parent.from.is_valid()) {
        from_parent.from.data.up = std::min(from_parent.from.data.up, from_parent.to.data.up);
        if (from_parent.to.data.up >= from_parent.from.data.depth) {
          /* Parent is an articulation point, drop off this subtree */
          // Pop off and paint all edges after this vertex
          from_parent.from.data.popped_off++;
          while ((int)current_comp.size() > from_parent.to.data.stack_size) {
            int edge = current_comp.top();
            current_comp.pop();
            id2comp[edge] = current_comp_num;
          }
