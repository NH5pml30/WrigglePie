...
class bipartite_graph : public graph<NODE_DATA, EDGE_DATA> {
  ...
  struct edge_iter_wrapper {
    using tuple_t = decltype(*std::declval<EdgeIter>());
    EdgeIter data;
    int n1;
...
graph_t read_graph(std::istream &in) {
  ...
  struct edge_supplier {
    ...
    edge_supplier(std::istream &in, int pos) : in(&in), pos(pos) {
    }
    edge_supplier(int pos) : in(nullptr), pos(pos) {
    }
    ...
    auto operator*() const {
      ...
      return std::make_tuple(cached_from, cached_to);
    }
  };

  return graph_t(n, m, edge_supplier(in, 0), edge_supplier(k));
}

int main() {
  graph_t g = read_graph(std::cin);
  std::cout << g.max_matching(noop) << std::endl;
  return 0;
}
