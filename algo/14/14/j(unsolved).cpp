...
struct big_integer {
 ...
 private:
  ...
  big_integer &place_wise(const big_integer &b, const binary_operator &action);
};
...
big_integer::place_t big_integer::get_or_default(ptrdiff_t at) const {
  if (at < 0)
    return 0;
  if (static_cast<size_t>(at) >= data.size())
...
big_integer &big_integer::place_wise(const big_integer &b, const binary_operator &action) {
  iterate(b, action);
  return shrink();
}
...
static type addc(type left, type right, bool &carry) {
  ...
  carry = (limit_max - left < right || (left == limit_max && carry == 1) ||
...
static inline uint16_t low_bytes(uint32_t x) {
  return x & 0xFFFF;
}
static inline uint16_t high_bytes(uint32_t x) {
...
static std::pair<uint64_t, uint64_t> mul(uint64_t left, uint64_t right) {
  ...
  return {bd + (ad << 32) + (bc << 32), ac + high_bytes(ad) + high_bytes(bc) + carry};
}
...
static bool less_3_digits(uint64_t lhs_low, uint32_t lhs_high, uint64_t rhs_low,
                          uint32_t rhs_high) {
...
static std::pair<uint32_t, uint64_t> div3_2(uint32_t lhs_low, uint32_t lhs_med, uint32_t lhs_high,
  ...
  uint64_t r_low64 = (uint64_t{lhs_med} << 32) | lhs_low, d = (uint64_t{rhs_high} << 32) | rhs_low;
  uint32_t r_high = lhs_high, d2 = rhs_high;
  ...
  if (less_3_digits(r_low64, r_high, dq.first << 16,
                    low_bytes((dq.second << 16) | (dq.first >> 48)))) {
    // correct estimate
    qt1--;
...
big_integer &big_integer::bit_shift(int rhs) {
  ...
  std::vector<place_t> new_data(std::max(data.size() + places + (bits > 0), size_t{1}),
                                get_or_default(rhs < 0 ? static_cast<ptrdiff_t>(data.size()) : -1));
  for (size_t i = std::max(places, 0); i < data.size() + places + (bits > 0); i++) {
    ...
    if (bits == 0)
      ...
      place_t lsrc = get_or_default(static_cast<ptrdiff_t>(i) - places),
...
std::istream &operator>>(std::istream &i, big_integer &a) {
  a = 0;
  int ch;
  while ((ch = i.get()) != EOF)
    if (std::isdigit(ch)) {
