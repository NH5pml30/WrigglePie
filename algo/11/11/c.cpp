...
void EdmondsKarp(const graph_t &g, int s, int t, capacity_t A) {
  std::vector<std::pair<int, const edge_data_t *>> prev(g.size());
  auto bfs = [&]() {
    std::queue<int> q;
    q.push(s);
    while (!q.empty()) {
      ...
      for (auto &next : g.nodes[v].adjacent) {
        int u = next.other_id;
        auto &vu = next.data;
        if (vu.capacity - vu.flow >= A && prev[u].second == nullptr) {
          prev[u].first = v;
          prev[u].second = &next.data;
          q.push(u);
        }
      }
    }

    if (prev[t].second == nullptr)
      return 0;

    capacity_t fp = std::numeric_limits<capacity_t>::max();
    int u = t;
    while (u != s) {
      fp = std::min(fp, prev[u].second->capacity - prev[u].second->flow);
      u = prev[u].first;
    }
    u = t;
    while (u != s) {
      auto &vu = *prev[u].second;
      auto &uv = *vu.paired;
      vu.flow += fp;
      uv.flow -= fp;
      u = prev[u].first;
  ...
  while (true) {
    std::fill(prev.begin(), prev.end(), std::make_pair(-1, nullptr));
    if (bfs() == 0)
      return;
  }
}

std::vector<bool> scaling(const graph_t &g, int s, int t, capacity_t A) {
  A--;
  A |= A >> 1;
  A |= A >> 2;
  A |= A >> 4;
  ...
  std::function<void(int)> dfs = [&](int v) {
    ...
    for (auto &next : g.nodes[v].adjacent) {
      auto &vu = next.data;
      int u = next.other_id;
      if (vu.capacity - vu.flow > 0 && !used[u])
        dfs(u);
    }
  };
  dfs(s);
...
int main() {
  ...
  std::vector<bool> S = scaling(g, 0, (int)g.size() - 1, A);
  int ss = 0;
