...
template<typename flow_cost_t, typename flow_t>
flow_cost_t flow_min_cost(const graph_t &gf, int s, int t, flow_t k) {
...
int main() {
  ...
  std::multimap<int, std::pair<int, int>> begins;
  for (int i = 0; i < n; i++) {
    std::cin >> tasks[i].begin >> tasks[i].end >> tasks[i].weight;
    tasks[i].end += tasks[i].begin - 1;
    begins.insert({tasks[i].begin, {i, -1}});
  }

  std::vector<edge_descriptor> edges;
  auto add_edge = [&](int from, int to, capacity_t capacity, cost_t cost) {
    edges.push_back({from, to, {nullptr, capacity, cost}});
    edges.push_back({to, from, {&std::get<2>(edges.back()), 0, -cost}});
  };
  for (int i = 0; i < n; i++) {
    add_edge(2 * i, 2 * i + 1, 1, -tasks[i].weight);  // execute task
  }
  int cur = 2 * n;
  for (auto &p : begins) {
    add_edge(cur, cur + 1, k, 0);  // skip task
    add_edge(cur, 2 * p.second.first, 1, 0);  // start task
    p.second.second = cur;
    cur++;
  }
  for (int i = 0; i < n; i++) {
    auto next_task = begins.upper_bound(tasks[i].end);
    int point = next_task == begins.end() ? cur : next_task->second.second;
    add_edge(2 * i + 1, point, 1, 0);  // finish task
  }

  graph_t g = graph_t(cur + 1, edges.begin(), edges.end());
  flow_min_cost<int64_t>(g, 2 * n, cur, k);

  for (int i = 0; i < n; i++)
    for (auto &next : g.nodes[2 * i].adjacent)
      if (next.other_id == 2 * i + 1)
        std::cout << next.data.flow << ' ';
  std::cout << std::endl;
