...
struct edge_outer_supplier : supplier_iterator<edge_outer_supplier, const edge_descriptor> {
  ...
  edge_outer_supplier(std::istream &in, int m) : cur(in), end(m) {}
  edge_outer_supplier(int m) : cur(m), end(m) {}
...
graph_t read_graph(std::istream &in) {
  ...
  return graph_t(n, edge_outer_supplier(in, m), edge_outer_supplier(m));
}

capacity_t FordFulkerson(const graph_t &g, int s, int t) {
  std::vector<bool> used(g.size());
  std::function<capacity_t(int, int, capacity_t)> dfs = [&](int v, int t, capacity_t c_min) {
    if (v == t)
      return c_min;
    used[v] = true;
    for (auto &next : g.nodes[v].adjacent) {
      ...
      if (vu.capacity - vu.flow > 0 && !used[u]) {
        capacity_t d = dfs(u, t, std::min(c_min, vu.capacity - vu.flow));
        if (d > 0) {
          vu.flow += d;
          uv.flow -= d;
          return d;
  ...
  while (true) {
    ...
    capacity_t d = dfs(s, t, std::numeric_limits<capacity_t>::max());
    if (d == 0)
      break;
    fv += d;
  }
...
int main() {
  ...
  std::cout << FordFulkerson(g, 0, (int)g.size() - 1) << std::endl;
  return 0;
}
