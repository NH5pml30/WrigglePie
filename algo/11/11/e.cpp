...
struct edge_outer_supplier : supplier_iterator<edge_outer_supplier, const edge_descriptor> {
  ...
  std::vector<edge_data_t *> &was_from, &was_to;
  capacity_t &max;

  edge_outer_supplier(std::istream &in, int m, std::vector<edge_data_t *> &was_from,
                      std::vector<edge_data_t *> &was_to, capacity_t &max)
      : cur(in), end(m), was_from(was_from), was_to(was_to), max(max) {}
  edge_outer_supplier(int m, std::vector<edge_data_t *> &was_from,
                      std::vector<edge_data_t *> &was_to, capacity_t &max)
      : cur(m), end(m), was_from(was_from), was_to(was_to), max(max) {}

  const edge_descriptor &invoke() const override {
    if (!data.has_value()) {
      switch (counter) {
      case 0:
        if (!was_to[std::get<1>(*cur) - 1]) {
          if (std::get<2>(*cur) > max)
            max = std::get<2>(*cur);
          data = edge_descriptor{0, std::get<1>(*cur),
                                  edge_data_t{&tracker, -1, std::get<2>(*cur), 0}};
          break;
        } else {
          was_to[std::get<1>(*cur) - 1]->capacity += std::get<2>(*cur);
          counter++;
          [[fallthrough]];
        }
      case 1:
        if (!was_to[std::get<1>(*cur) - 1]) {
          was_to[std::get<1>(*cur) - 1] = tracker.paired;
          data = edge_descriptor{std::get<1>(*cur), 0, edge_data_t{tracker.paired, -1, 0, 0}};
          break;
        } else {
          counter++;
          [[fallthrough]];
      ...
      case 2:
        if (!was_from[std::get<0>(*cur) - 1]) {
          data = edge_descriptor{std::get<0>(*cur), (int)was_from.size() + 1,
                                 edge_data_t{&tracker, -1, std::get<2>(*cur), 0}};
          break;
        } else {
          was_from[std::get<0>(*cur) - 1]->capacity += std::get<2>(*cur);
          counter++;
          [[fallthrough]];
        }
      case 3:
        if (!was_from[std::get<0>(*cur) - 1]) {
          was_from[std::get<0>(*cur) - 1] = tracker.paired;
          data = edge_descriptor{(int)was_from.size() + 1, std::get<0>(*cur),
                                 edge_data_t{tracker.paired, -1, 0, 0}};
          break;
        } else {
          counter++;
          [[fallthrough]];
      ...
      case 4:
        if (std::get<3>(*cur) - std::get<2>(*cur) > max)
          max = std::get<3>(*cur) - std::get<2>(*cur);
        data =
            edge_descriptor{std::get<0>(*cur), std::get<1>(*cur),
                            edge_data_t{&tracker, cur.istream_iterator::sup_base_t::cur,
                                        std::get<3>(*cur) - std::get<2>(*cur), std::get<2>(*cur)}};
        break;
      case 5:
        data = edge_descriptor{
            std::get<1>(*cur), std::get<0>(*cur),
            edge_data_t{tracker.paired, -1, 0, 0}};
        break;
      }
    }
...
std::tuple<graph_t, int, capacity_t> read_graph(std::istream &in) {
  int n, m;
  if (!(in >> n >> m)) {
    edge_descriptor *it = nullptr;
    return {graph_t(0, it, it), 0, 0};
  }

  capacity_t A = 0;
  std::vector<edge_data_t *> was_to(n), was_from(n);
  graph_t g = graph_t(n + 2, edge_outer_supplier(in, m, was_from, was_to, A),
                      edge_outer_supplier(m, was_from, was_to, A));
  return {std::move(g), m, A};
}
...
ResT Dinic(const graph_t &g, int s, int t, capacity_t A) {
  ...
  using adj_iterator = typename decltype(g.nodes[0].adjacent)::const_iterator;
  std::vector<adj_iterator> h(g.size());
  std::function<capacity_t(int, capacity_t)> dfs = [&](int v, capacity_t c_min) {
    if (v == t)
      return c_min;
    for (; h[v] != g.nodes[v].adjacent.end(); ++h[v]) {
      int u = h[v]->other_id;
      auto &vu = h[v]->data;
      auto &uv = *vu.paired;
...
int main() {
  ...
  auto[g, m, A] = read_graph(std::cin);
  scaling<capacity_t>(g, 0, (int)g.size() - 1, A);

  for (auto &next : g.nodes[0].adjacent) {
    if (next.data.flow != next.data.capacity) {
  ...
  std::function<void(int)> dfs = [&](int v) {
    ...
    for (auto &next : g.nodes[v].adjacent) {
      if (next.data.id != -1)
        res[next.data.id] = next.data.flow + next.data.l;
      dfs(next.other_id);
    }
  };
  dfs(0);
