...
int main() {
  ...
  for (int i = 0; i < 10000; i++) {
    int n = rand() % 2000 + 1;
    edge_descriptor *it = nullptr;
    graph_t g(n, it, it, supplier_iterator_t<int>(0), supplier_iterator_t<int>(n));
    for (int v = 1; v < n; v++) {
      int u = rand() % v;
      g.nodes[v].add_adjacent(u, {});
      g.nodes[u].add_adjacent(v, {});
      g.n_of_edges_++;
    }

    int m = rand() % 100000 - (n - 1);
    while (m-- > 0) {
      int u = rand() % n, v = rand() % n;
      if (u != v && std::find_if(g.nodes[v].adjacent.begin(), g.nodes[v].adjacent.end(), [u](auto &edge) {
            return edge.other_id == u;
          }) == g.nodes[v].adjacent.end()) {
        g.nodes[v].add_adjacent(u, {});
        g.nodes[u].add_adjacent(v, {});
        g.n_of_edges_++;
      }
    ...
    int k = std::min(rand() % 40 + 1, n);

    auto cover = vertex_cover(g, k);
    auto ideal_cover = ideal::vertex_cover(g, k);

    if (cover.empty() != ideal_cover.empty()) {
    ...
    auto check_covered = [](const auto &g, auto &cover) {
      int n_covered = 0;
      for (int v = 0; v < (int)g.nodes.size(); v++)
        if (cover[v])
          for (auto &[other_id, data] : g.nodes[v].adjacent) {
            n_covered += !cover[other_id] || (cover[other_id] && other_id < v);
            [[maybe_unused]] auto d = data;
          }
      return n_covered == g.n_of_edges();
    };

    if (!check_covered(g, cover)) {
      std::cout << "WRONG COVER" << std::endl;
      return 0;
    }
